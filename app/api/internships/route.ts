...
export async function GET(req: NextRequest) {
  try {
    ...
    if (stipendMin || stipendMax) {
      ...
      if (stipendMin) filter["stipend.amount"].$gte = parseInt(stipendMin);
      if (stipendMax) filter["stipend.amount"].$lte = parseInt(stipendMax);
    }

    if (searchParams.get("remote") === "true") filter.isRemote = true;

    const locations = searchParams.getAll("location").filter((l) => l !== "Work from Home");
    ...
    if (durationMin || durationMax) {
      ...
      if (durationMin) filter["duration.value"].$gte = parseInt(durationMin);
      if (durationMax) filter["duration.value"].$lte = parseInt(durationMax);
    }
    ...
    const sort: Record<string, any> =
      sortBy === "stipend"   ? { "stipend.amount": -1 } :
      sortBy === "deadline"  ? { deadlineDate: 1 }      :
                               { datePublished: -1 };
