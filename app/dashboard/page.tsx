...
import { ProfileSkeleton, InternshipCardSkeleton } from "@/components/ui/Skeleton";
  ...
  return (
    ...
    <div className="h-screen bg-gray-50 flex flex-col overflow-hidden">
      ...
      <div className="lg:hidden flex-shrink-0 z-30 bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between">
        ...
        <span className="font-semibold text-gray-900 text-sm">{TAB_TITLES[activeTab]}</span>
        <button
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <main className="flex-1 overflow-y-auto pb-16 lg:pb-0">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
            ...
            {activeTab === "internships" && <InternshipsTab user={user} />}
            {activeTab === "resume" && <ResumeTab user={user} />}
            {activeTab === "profile" && <ProfileTab user={user} />}
            {activeTab === "saved" && user && <SavedTab user={user} />}
          </div>
        </main>
      </div>
...
function SavedTab({ user }: { user: User }) {
  ...
  useEffect(() => {
    if (!user.savedInternships.length) { setLoading(false); return; }
  ...
  return (
    <div>
      <p className="text-sm text-gray-500 mb-4">{internships.length} saved internships</p>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-xl flex items-center justify-between">
          <p className="text-sm text-blue-700">
            <strong>Sign in</strong> to apply, save internships, and get AI-powered recommendations.
          </p>
          <Link href="/login" className="text-sm font-medium text-blue-700 underline ml-4 flex-shrink-0">
            Login →
          </Link>
        </div>
