    ...
    <aside className="w-64 flex-shrink-0 bg-white border-r border-gray-200 flex flex-col h-full">
      ...
      <div className="p-5 border-b border-gray-100">
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-xs font-semibold text-blue-600">{user.profileCompletionScore}%</span>
          </div>
      ...
      <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
        {NAV_ITEMS.map(({ tab, label, icon: Icon }) => (
          ...
          >
            <Icon className={cn("h-4 w-4", activeTab === tab ? "text-blue-600" : "text-gray-400 group-hover:text-gray-600")} />
            <span className="flex-1">{label}</span>
            {activeTab === tab && <ChevronRight className="h-3 w-3 text-blue-400" />}
          </Link>
        ))}
      </nav>
