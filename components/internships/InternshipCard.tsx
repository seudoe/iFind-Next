...
import { MapPin, Clock, IndianRupee, Bookmark, BookmarkCheck, ExternalLink, Wifi, Sparkles } from "lucide-react";
...
export function InternshipCard({
  ...
  const handleSave = (e: React.MouseEvent) => {
    ...
    setSaved(next);
    // Call real API
    fetch(`/api/internships/${internship._id}/save`, {
  ...
  const handleApply = async (e: React.MouseEvent) => {
    ...
    setApplying(true);
    try {
      const res = await fetch(`/api/internships/${internship._id}/apply`, {
        method: "POST",
        credentials: "include",
      });
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            ...
            {isRecommended && (
              <Badge variant="default" className="flex-shrink-0 flex items-center gap-1">
        ...
        <div className="hidden md:block text-xs text-right flex-shrink-0">
          {internship.deadlineDate ? (
            <span className={cn(deadlineSoon ? "text-red-600 font-medium" : "text-gray-500")}>
              {deadlineSoon ? "⚡ " : ""}Deadline: {formatDate(internship.deadlineDate)}
            </span>
          ) : (
  ...
  return (
    ...
    >
      <div className="p-5 flex-1">
        ...
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">{internship.company}</p>
            </div>
          </div>
          <button
            onClick={handleSave}
        ...
        {/* Deadline */}
        {internship.deadlineDate && (
          <p className={cn("text-xs mt-2", deadlineSoon ? "text-red-600 font-medium" : "text-gray-400")}>
